...
const { tambahEntriDns, hapusEntriDns, removeAllDNSEntries, removeAllDNSEntriesSync, cekSemuaStatusDns, TOOL_HOSTS, apakahSudoTersedia, apakahSandiSudoDiperlukan } = require("./dns/dnsConfig");
const { isAdmin } = require("./winElevated.js");

const IS_WIN = process.platform === "win32";
const IS_MAC = process.platform === "darwin";
const { buatSertifikat } = require("./cert/generate");
const { pasangSertifikat, unpasangSertifikat } = require("./cert/install");
...
// (prevents EBUSY on `npm i -g nexroute@latest` while MITM is running).
...
function killProcess(pid, force = false, sudoPassword = null) {
  if (IS_WIN) {
    ...
    if (sudoPassword || apakahSudoTersedia()) {
      const { eksekusiDenganSandi } = require("./dns/dnsConfig");
      eksekusiDenganSandi(cmd, sudoPassword || "").catch(() => exec(cmd, { windowsHide: true }, () => { }));
    } else {
...
function enkripsiSandi(plaintext) {
  const key = deriveKey();
...
async function simpanPengaturanMitm(enabled, password) {
  if (!_updateSettings) return;
  try {
    const updates = { mitmEnabled: enabled };
    if (password) updates.mitmSudoEncrypted = enkripsiSandi(password);
...
async function muatSandiTerenkripsi() {
  if (!_getSettings) return null;
  try {
    const settings = await _getSettings();
    if (!settings.mitmSudoEncrypted) return null;
    return dekripsiSandi(settings.mitmSudoEncrypted);
  } catch {
    return null;
  }
}
...
async function restoreToolDNS(sudoPassword) {
  ...
  const password = sudoPassword || getCachedPassword() || await muatSandiTerenkripsi();
  for (const [tool, enabled] of Object.entries(state)) {
    if (!enabled || !TOOL_HOSTS[tool]) continue;
    try {
      await tambahEntriDns(tool, password);
    } catch (e) {
...
async function hasDnsPrivilege() {
  ...
  if (!apakahSandiSudoDiperlukan()) return true;
  const pwd = getCachedPassword() || await muatSandiTerenkripsi();
  return !!pwd;
}
...
async function killLeftoverMitm(sudoPassword) {
  ...
  if (!IS_WIN && SERVER_PATH) {
    try {
      ...
      if (sudoPassword || apakahSudoTersedia()) {
        const { eksekusiDenganSandi } = require("./dns/dnsConfig");
        await eksekusiDenganSandi(`pkill -SIGKILL -f "${escaped}" 2>/dev/null || true`, sudoPassword || "").catch(() => { });
      } else {
...
async function getMitmStatus() {
  ...
  const dnsStatus = cekSemuaStatusDns();
  const rootCACertPath = path.join(MITM_DIR, "rootCA.crt");
  const certExists = fs.existsSync(rootCACertPath);
  const { cekSertifikatTerpasang } = require("./cert/install");
  const certTrusted = certExists ? await cekSertifikatTerpasang(rootCACertPath) : false;
...
async function scheduleMitmRestart(apiKey) {
  ...
  try {
    ...
    const password = getCachedPassword() || await muatSandiTerenkripsi();
    if (!password && !IS_WIN) {
      err("No cached password, cannot auto-restart");
      mitmIsRestarting = false;
      return;
    }
    await mulaiServer(apiKey, password);
...
async function killPort443Owner(owner, sudoPassword) {
  ...
  if (IS_WIN) {
    ...
    try {
      const { eksekusiDenganSandi } = require("./dns/dnsConfig");
      if (sudoPassword || apakahSudoTersedia()) {
        await eksekusiDenganSandi(`kill -9 ${owner.pid}`, sudoPassword || "");
      } else {
...
async function mulaiServer(apiKey, sudoPassword, forceKillPort443 = false) {
  if (!serverProcess || serverProcess.killed) {
    try {
      if (fs.existsSync(PID_FILE)) {
        const savedPid = parseInt(fs.readFileSync(PID_FILE, "utf-8").trim(), 10);
        if (savedPid && isProcessAlive(savedPid)) {
          ...
          await simpanPengaturanMitm(true, sudoPassword);
  ...
  // Atomically claim lock to prevent concurrent mulaiServer across processes.
  ...
  if (!certExists || isCertExpired(rootCACertPath)) {
    if (certExists) {
      ...
      const password = sudoPassword || getCachedPassword() || await muatSandiTerenkripsi();
      try { await unpasangSertifikat(password, rootCACertPath); } catch { /* best effort */ }
    }
    log(" Generating Root CA...");
    await buatSertifikat();
  }

  // Step 1.5: Auto-install Root CA if not trusted yet
  const { cekSertifikatTerpasang } = require("./cert/install");
  const rootCATrusted = await cekSertifikatTerpasang(rootCACertPath);
  const linuxNoSystemTrust = !IS_WIN && !IS_MAC && !apakahSudoTersedia();
  if (!rootCATrusted) {
    ...
    if (linuxNoSystemTrust) {
      ...
      if (!password && apakahSandiSudoDiperlukan()) {
        throw new Error("Sudo password required to install Root CA certificate");
      }
      try {
        await pasangSertifikat(password, rootCACertPath);
        log(" Cert:  trusted");
      } catch (e) {
 ...
 */
async function hentikanServer(sudoPassword) {
  ...
  if (IS_WIN) {
    ...
    try {
      const { isAdmin, jalankanPowershellAdmin, quotePs } = require("./winElevated.js");
      if (isAdmin()) {
        ...
        `;
        await jalankanPowershellAdmin(script);
      }
  ...
  try { fs.unlinkSync(PID_FILE); } catch { /* ignore */ }
  try { fs.unlinkSync(LOCK_FILE); } catch { /* ignore */ }
  await simpanPengaturanMitm(false, null);
  mitmIsRestarting = false;
 ...
 */
async function aktifkanDnsAlat(tool, sudoPassword) {
 ...
 */
async function nonaktifkanDnsAlat(tool, sudoPassword) {
  const password = sudoPassword || getCachedPassword() || await muatSandiTerenkripsi();
  await hapusEntriDns(tool, password);
...
async function trustCert(sudoPassword) {
  ...
  const { pasangSertifikat } = require("./cert/install");
  if (!IS_WIN && !IS_MAC && !apakahSudoTersedia()) {
    log(` Cert: system trust unavailable (no sudo). Use file: ${rootCACertPath}`);
    return;
  }
  const password = sudoPassword || getCachedPassword() || await muatSandiTerenkripsi();
  if (!password && apakahSandiSudoDiperlukan()) throw new Error("Sudo password required to trust certificate");
  await pasangSertifikat(password, rootCACertPath);
  if (password) setCachedPassword(password);
}

// Legacy aliases for backward compatibility
const startMitm = mulaiServer;
const stopMitm = hentikanServer;
...
module.exports = {
  ...
  getCachedPassword,
  setCachedPassword,
  muatSandiTerenkripsi,
  clearEncryptedPassword,
  apakahSandiSudoDiperlukan,
  initDbHooks,
  restoreToolDNS,
  hasDnsPrivilege,
