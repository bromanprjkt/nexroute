...
const URL_PATTERNS = {
  ...
  // see apakahRequestChat() for the header-based match.
...
/**
 * Whether this request is a chat turn we should intercept (vs teruskan).
 ...
 */
function apakahRequestChat(tool, req) {
...
const MODEL_PATTERNS = {
  ...
  claudeCode: [
    ...
    { match: /opus/i, alias: "claude-opus-4-6-thinking" },
    { match: /haiku/i, alias: "claude-haiku-4-6" },
  ],
};

// Models that must NEVER be re-routed — always teruskan to the real upstream, even when
...
// it through NexRoute to an external chat model makes typing laggy and burns provider quota per
...
function dapatkanAlatDariHost(host) {
  ...
  if (h === "api.anthropic.com") return "claudeCode";
  return null;
}
...
function ekstrakModel(url, body) {
...
module.exports = { IS_DEV, LSOF_BIN, TARGET_HOSTS, URL_PATTERNS, MODEL_SYNONYMS, MODEL_PATTERNS, MODEL_NO_MAP, LOG_BLACKLIST_URL_PARTS, dapatkanAlatDariHost, apakahRequestChat, ekstrakModel };
