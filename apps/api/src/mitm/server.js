...
const { log, err, simpanRequest, buatPenyimpanRespons, bersihkanFolderDump } = require("./logger");
const { IS_DEV, LSOF_BIN, TARGET_HOSTS, URL_PATTERNS, MODEL_SYNONYMS, MODEL_PATTERNS, MODEL_NO_MAP, dapatkanAlatDariHost, apakahRequestChat, ekstrakModel } = require("./config");
const { DATA_DIR, MITM_DIR } = require("./paths");
const { buatSertifikat, getCertForDomain } = require("./cert/generate");
...
const handlers = {
  ...
  claudeCode: require("./handlers/claudeCode"),
};
...
async function cariIpTarget(hostname) {
...
function kumpulkanBodyMentah(req) {
...
function dapatkanModelTujuan(tool, model) {
 ...
 */
async function teruskan(req, res, bodyBuffer, onResponse) {
  ...
  const dumper = ENABLE_FILE_LOG ? buatPenyimpanRespons(req, "teruskan") : null;

  const tool = dapatkanAlatDariHost(req.headers.host);
  ...
  try {
    const proto = await negosiasiAlpn(targetHost);
    if (proto === "h2") {
      return await teruskanHttp2(req, res, bodyForForwarding, headersForForwarding, targetHost, onResponse, dumper);
    }
  } catch (e) {
    err(`[mitm] ALPN negotiate failed: ${e.message}, fallback to HTTP/1.1`);
  }

  return teruskanHttps(req, res, bodyForForwarding, headersForForwarding, targetHost, onResponse, dumper);
}

// ── ALPN negotiation cache ────────────────────────────────────
const alpnCache = new Map(); // host → "h2" | "http/1.1"
async function negosiasiAlpn(host) {
  if (alpnCache.has(host)) return alpnCache.get(host);
  const ip = await cariIpTarget(host);
  return new Promise((resolve, reject) => {
...
// HTTP/2 teruskan using node:http2 native
async function teruskanHttp2(req, res, bodyBuffer, headers, targetHost, onResponse, dumper) {
  const targetIP = await cariIpTarget(targetHost);
...
async function teruskanHttps(req, res, bodyBuffer, headers, targetHost, onResponse, dumper) {
  const targetIP = await cariIpTarget(targetHost);
...
const server = https.createServer(sslOptions, async (req, res) => {
  try {
    ...
    const bodyBuffer = await kumpulkanBodyMentah(req);
    if (ENABLE_FILE_LOG) simpanRequest(req, bodyBuffer, "raw");

    // Anti-loop: lewati permintaan dari NexRoute
    if (req.headers[INTERNAL_REQUEST_HEADER.name] === INTERNAL_REQUEST_HEADER.value) {
      return teruskan(req, res, bodyBuffer);
    }

    const tool = dapatkanAlatDariHost(req.headers.host);
    if (!tool) return teruskan(req, res, bodyBuffer);

    // Kiro IDE posts chat to `/` with x-amz-target (not path /generateAssistantResponse)
    if (!apakahRequestChat(tool, req)) return teruskan(req, res, bodyBuffer);
    ...
    if (tool === "cursor") {
      return handlers[tool].intercept(req, res, bodyBuffer, null, teruskan);
    }

    const model = ekstrakModel(req.url, bodyBuffer);

    // Kesengajaan teruskan: beberapa model tidak boleh dialihkan (e.g. Antigravity
    ...
    const mappedModel = dapatkanModelTujuan(tool, model);
    if (!mappedModel) {
      return teruskan(req, res, bodyBuffer);
    }

    return handlers[tool].intercept(req, res, bodyBuffer, mappedModel, teruskan);
  } catch (e) {
