const { err, buatPenyimpanRespons } = require("../logger");
const { IS_DEV } = require("../config");
const { ambilRouter, alirkanSse } = require("./base");
...
async function intercept(req, res, bodyBuffer, mappedModel) {
  const dumper = IS_DEV ? buatPenyimpanRespons(req, "intercept-antigravity") : null;
  const isStream = req.url.includes(":streamGenerateContent");
  try {
    const body = JSON.parse(bodyBuffer.toString());
    if (body.model) body.model = mappedModel;

    const routerRes = await ambilRouter(body, "/v1/chat/completions", req.headers);
    await alirkanSse(routerRes, res, dumper);
  } catch (error) {
