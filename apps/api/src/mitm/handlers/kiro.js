...
const { ambilRouter, pipeTransformedEventStream } = require("./base");
const fs = require("fs");
 ...
 * If nexroute's Anthropic→OpenAI conversion passes the input as a pre-parsed object,
 ...
 *   3. Forward to nexroute /v1/chat/completions (OpenAI SSE)
...
async function intercept(req, res, bodyBuffer, mappedModel) {
  try {
    ...
    if (isBinaryEventStream(bodyBuffer)) {
      ...
      throw new Error(`Binary EventStream format detected (${bodyBuffer.length}B) - request should use teruskan instead of intercept`);
    }
    ...
    // 3: Forward to nexroute
    const routerRes = await ambilRouter(openaiBody, "/v1/chat/completions", req.headers);
