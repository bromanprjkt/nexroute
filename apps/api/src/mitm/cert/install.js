...
const { eksekusiDenganSandi, apakahSudoTersedia } = require("../dns/dnsConfig.js");
const { jalankanPowershellAdmin, quotePs } = require("../winElevated.js");
...
const ROOT_CA_CN = "NexRoute MITM Root CA";
 ...
 */
async function cekSertifikatTerpasang(certPath) {
  if (IS_WIN) return cekSertifikatTerpasangWindows(certPath);
  if (IS_MAC) return cekSertifikatTerpasangMac(certPath);
  return cekSertifikatTerpasangLinux();
}

function cekSertifikatTerpasangMac(certPath) {
...
function cekSertifikatTerpasangWindows(certPath) {
  return new Promise((resolve) => {
 ...
 */
async function pasangSertifikat(sudoPassword, certPath) {
  if (!fs.existsSync(certPath)) {
    throw new Error(`Certificate file not found: ${certPath}`);
  }

  const isInstalled = await cekSertifikatTerpasang(certPath);
  if (isInstalled) {
  ...
  if (IS_WIN) {
    await pasangSertifikatWindows(certPath);
  } else if (IS_MAC) {
    await pasangSertifikatMac(sudoPassword, certPath);
  } else {
    await pasangSertifikatLinux(sudoPassword, certPath);
  }
}

async function pasangSertifikatMac(sudoPassword, certPath) {
  // Remove all old certs with same name first to avoid duplicate/stale cert conflict
  const deleteOld = `security delete-certificate -c "NexRoute MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
  const install = `security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain "${certPath}"`;
  try {
    await eksekusiDenganSandi(`${deleteOld} && ${install}`, sudoPassword);
...
async function pasangSertifikatWindows(certPath) {
 ...
 */
async function unpasangSertifikat(sudoPassword, certPath) {
  const isInstalled = await cekSertifikatTerpasang(certPath);
  if (!isInstalled) {
  ...
  if (IS_WIN) {
    await unpasangSertifikatWindows();
  } else if (IS_MAC) {
    await unpasangSertifikatMac(sudoPassword, certPath);
  } else {
    await unpasangSertifikatLinux(sudoPassword);
  }
}

async function unpasangSertifikatMac(sudoPassword, certPath) {
  const fingerprint = getCertFingerprint(certPath).replace(/:/g, "");
  const command = `security delete-certificate -Z "${fingerprint}" /Library/Keychains/System.keychain`;
  try {
    await eksekusiDenganSandi(command, sudoPassword);
...
async function unpasangSertifikatWindows() {
...
function cekSertifikatTerpasangLinux() {
...
async function updateNssDatabases(certPath, action = 'add') {
  const certName = "NexRoute MITM Root CA";
...
async function pasangSertifikatLinux(sudoPassword, certPath) {
  if (!apakahSudoTersedia()) {
  ...
  try {
    await eksekusiDenganSandi(cmd, sudoPassword);
    await updateNssDatabases(certPath, 'add');
...
async function unpasangSertifikatLinux(sudoPassword) {
...
module.exports = { pasangSertifikat, unpasangSertifikat, cekSertifikatTerpasang };
