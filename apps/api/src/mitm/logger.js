...
function dekodeBody(buf, encoding) {
...
function simpanRequest(req, bodyBuffer, tag = "raw") {
...
function buatPenyimpanRespons(req, tag = "raw") {
  if (isBlacklisted(req.url)) return null;
  const ts = new Date().toISOString().replace(/[:.]/g, "-");
  const slug = slugify((req.headers.host || "") + req.url);
  ...
  return {
    ...
    writeChunk: (chunk) => {
      ...
      try {
        ...
        const decoded = dekodeBody(raw, enc);
...
module.exports = { log, err, simpanRequest, buatPenyimpanRespons, bersihkanFolderDump };
