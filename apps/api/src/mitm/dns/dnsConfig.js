...
const { jalankanPowershellAdmin, isAdmin } = require("../winElevated.js");
...
function canRunSudoWithoutPassword() {
  if (IS_WIN || !apakahSudoTersedia()) return true;
  try {
...
function apakahSandiSudoDiperlukan() {
  return !IS_WIN && apakahSudoTersedia() && !canRunSudoWithoutPassword();
}
 ...
 */
function eksekusiDenganSandi(command, password) {
  return new Promise((resolve, reject) => {
    const useSudo = apakahSudoTersedia();
    const child = useSudo
 ...
 */
async function bersihkanCacheDns(sudoPassword) {
  if (IS_WIN) return; // Windows flushes inline via ipconfig
  if (IS_MAC) {
    await eksekusiDenganSandi("dscacheutil -flushcache && killall -HUP mDNSResponder", sudoPassword);
  } else {
    await eksekusiDenganSandi("resolvectl flush-caches 2>/dev/null || true", sudoPassword);
  }
}
 ...
 */
async function tambahEntriDns(tool, sudoPassword) {
  ...
  try {
    if (IS_WIN) {
      ...
      await jalankanPowershellAdmin("ipconfig /flushdns | Out-Null");
    } else {
      const current = fs.readFileSync(HOSTS_FILE, "utf8");
      const trimmed = current.replace(/[\r\n\s]+$/g, "");
      const toAppend = entriesToAdd.map(h => `127.0.0.1 ${h}`).join("\n");
      ...
      await eksekusiDenganSandi(`printf '%s' '${escaped}' | tee ${HOSTS_FILE} > /dev/null`, sudoPassword);
      await bersihkanCacheDns(sudoPassword);
    }
 ...
 */
async function hapusEntriDns(tool, sudoPassword) {
  const hosts = TOOL_HOSTS[tool];
  if (!hosts) throw new Error(`Unknown tool: ${tool}`);
...
async function removeAllDNSEntries(sudoPassword) {
  for (const tool of Object.keys(TOOL_HOSTS)) {
    try {
      await hapusEntriDns(tool, sudoPassword);
    } catch (e) {
